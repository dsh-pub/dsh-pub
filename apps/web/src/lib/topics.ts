...
export interface RegistryTopic {
  id: RegistryTopicId;
  label: LocalizedTopicText;
  description: LocalizedTopicText;
  selectionAdvice: LocalizedTopicText;
...
export function localizedTopic(value: LocalizedTopicText, locale: Locale): string {
  return value[locale];
}

export function topicForEntry(entry: CatalogEntry): RegistryTopic {
  const topic = registryTopics.find((candidate) => {
    const categories: readonly string[] = candidate.sourceCategories;
    return categories.includes(entry.category);
  });
  if (!topic) throw new Error(`No registry topic for category: ${entry.category}`);
  return topic;
}
...
export function entriesForTopic(
  ...
  const categories: readonly string[] = topic.sourceCategories;
